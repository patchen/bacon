...
if __name__ == "__main__":
    ...
    if hasattr(bacon, 'parse_actor_data'):
        import StringIO
...
''')
        ...
        result = bacon.parse_actor_data(reader)
        if expected != result:
    ...
    if hasattr(bacon, 'invert_actor_dict'):
        expected = {"A Few Good Men (1992)": ["Kevin Bacon"],
                    "Sleepers (1996)": ["Kevin Bacon"]}
        result = bacon.invert_actor_dict({"Kevin Bacon": ["A Few Good Men (1992)",
                                                          "Sleepers (1996)"]})
        if expected != result:
    ...
    if hasattr(bacon, 'find_connection'):
        expected = [('Sleepers (1996)', 'Kevin Bacon')]
        result = bacon.find_connection("Robert De Niro", 
